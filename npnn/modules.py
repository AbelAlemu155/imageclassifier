...
class ELU(Module):
    ...
    def forward(self, x, train=True):
        ...
        self.x = x # store the input for later use
    ...
    def backward(self, grad):
        ...
        """
        # print(self.x.shape)
        arr = np.where(self.x > 0, 1, self.alpha * (np.exp(self.x)))
...
class Dense(Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        W = np.random.uniform(low=-u, high=u, size=(dim_out, dim_in))
        b = np.zeros(dim_out)
    ...
    def backward(self, grad):
        ...
        # TODO
        # W.grad = dL/dW_k = ?
        W.grad = np.dot(grad.T,self.x) / self.x.shape[0]
        # b.grad = dL/db_k = ?
        b.grad =  np.sum(grad, axis=0)/ self.x.shape[0]
        # dx = dL/dx_{k-1} = ?
